...
public:
    ...
    void updated_solution(solution_t &solution, const discrete_mode_t &mode) const override
    {
        ...
        // Use pre-computed total electrical power (heater + fan) from mode
        const double energy_elec = mode.total_electrical_power * time_delta;
        // Only count electrical energy consumption (not thermal energy produced)
        solution.resources.energy += energy_elec;
        // Update time
