...
///
/// @brief Implements Particle Swarm Optimization (PSO) for refining solutions
/// and optimizing Pareto fronts.
///
/// @details
/// This file provides the core PSO search functions used for optimizing
/// mode execution sequences in a simulation-based framework. It includes:
/// - Functions for initializing particles and updating their velocities
///   and positions based on inertia, cognitive, and social components.
/// - Evaluation methods for determining the fitness of particles.
/// - Optimization routines for refining solutions, Pareto fronts, and
///   full optimization results.
///
/// The PSO implementation follows a swarm-based heuristic approach, iteratively
/// improving solutions based on personal and global bests. It is particularly
/// useful in multi-objective optimization scenarios where trade-offs between
/// competing objectives need to be managed.
///
...
/// @brief Provides Particle Swarm Optimization (PSO) functions and structures.
///
/// @details This namespace includes algorithms and data structures for
/// optimizing solutions using Particle Swarm Optimization. It provides
/// mechanisms for refining Pareto-optimal fronts, tuning search parameters,
/// and improving solution quality by leveraging swarm intelligence.
namespace pso
{
...
///
/// @return A tuple containing a random number generator and a uniform integer
/// distribution.
...
///
/// @param parameters PSO parameters, including inertia, cognitive, and social
/// weights.
/// @param personal_best The personal best number of executions for the given
/// mode.
...
inline void update_particle_velocity_and_position(
    ...
    const flexman::core::ModeExecution &personal_best,
    const flexman::core::ModeExecution &global_best,
    double &velocity,
    flexman::core::ModeExecution &particle)
{
...
/// space.
///
/// @param parameters PSO parameters guiding the update, including inertia,
...
/// @param velocities Vector of velocities for each particle, representing the
/// rate of change in execution counts.
/// @param particles Vector of particles, where each particle contains a
/// sequence of mode executions to be updated.
inline void update_all_particles(
    const SolverParameters &parameters,
    const std::vector<std::vector<flexman::core::ModeExecution>> &personal_best,
    const std::vector<flexman::core::ModeExecution> &global_best,
    std::vector<std::vector<double>> &velocities,
    std::vector<std::vector<flexman::core::ModeExecution>> &particles)
{
...
/// @param personal_best The current personal best sequence of mode executions
/// for the particle.
/// @param personal_best_fitness The fitness value of the particle's personal
/// best.
/// @param global_best The current global best sequence of mode executions
/// across all particles.
/// @param global_best_fitness The fitness value of the global best.
///
/// @return True if the solution generated by the particle is valid (complete);
/// otherwise, false.
template <typename State, typename Mode, typename Resources>
auto evaluate_particle(
    const flexman::core::Manager<State, Mode, Resources> *manager,
    const std::vector<Mode> &modes,
    const std::vector<flexman::core::ModeExecution> &particle,
    std::vector<flexman::core::ModeExecution> &personal_best,
    std::vector<flexman::core::ModeExecution> &global_best,
    double &personal_best_fitness,
    double &global_best_fitness) -> bool
{
...
auto optimize_solution(
    ...
    const SolverParameters &parameters,
    const std::vector<Mode> &modes,
    const flexman::core::Solution<State, Resources> &initial_solution)
{
    // Initialize containers for particles, personal bests, and velocities.
    std::vector<std::vector<flexman::core::ModeExecution>> particles(parameters.num_particles);
    std::vector<std::vector<flexman::core::ModeExecution>> personal_best(parameters.num_particles);
    ...
    std::vector<flexman::core::ModeExecution> global_best = initial_solution.sequence;
...
/// @brief Optimizes a Pareto front using the provided manager and solver
/// parameters.
///
/// @tparam State The type representing the system's state.
/// @tparam Mode The type representing the system's mode.
...
auto optimize_pareto_front(
    ...
    const SolverParameters &parameters,
    const std::vector<Mode> &modes,
    const flexman::core::ParetoFront<State, Resources> &pareto_front)
{
    flexman::core::ParetoFront<State, Resources> optimized = {
        .solutions           = {},
    ...
    std::size_t index = 1;
    std::size_t total = pareto_front.solutions.size();
...
/// @brief Optimizes a result object using the provided manager and solver
/// parameters.
///
/// @tparam State The type representing the system's state.
/// @tparam Mode The type representing the system's mode.
...
auto optimize_result(
    ...
    const SolverParameters &parameters,
    const std::vector<Mode> &modes,
    const flexman::core::Result<State, Resources> &result)
{
    flexman::core::Result<State, Resources> optimized = {
        .pareto_fronts = {},
    };

    std::size_t index = 1;
    std::size_t total = result.pareto_fronts.size();
